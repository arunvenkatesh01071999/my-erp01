...
function closingCashRepo(fastify) {
  async function getclosingCashReport({ body, params, logTrace }) {
    const knex = this;
    const query = knex
      .select([
        `${CLOSINGCASHMST.NAME}.*`,
      ...
      .leftJoin(
        ...
        `${CLOSINGCASHMST.NAME}.${CLOSINGCASHMST.COLUMNS.SALESMAN_ID}`,
        `${SALESMAN.NAME}.${SALESMAN.COLUMNS.ID}`
      )

      .whereRaw(
        `DATE(${CLOSINGCASHMST.NAME}.${CLOSINGCASHMST.COLUMNS.DATE}) >= ?`,
        [body.from_date]
      )
      .whereRaw(
        `DATE(${CLOSINGCASHMST.NAME}.${CLOSINGCASHMST.COLUMNS.DATE}) <= ?`,
      ...
      .orderBy(`${CLOSINGCASHMST.NAME}.${CLOSINGCASHMST.COLUMNS.ID}`, "DESC");
  ...
  async function getOneClosingCashReport({ body, params, logTrace }) {
    const knex = this;
    const query = knex
      .select([
        `${CLOSINGCASHMST.NAME}.*`,
  ...
  async function closingCashEmail({ body, params, logTrace }) {
    const knex = this;
    const query = knex
      .select([
        `${CLOSINGCASHMST.NAME}.*`,
  ...
  return {
    getclosingCashReport,
    closingCashEmail,
    getOneClosingCashReport
  };
}

module.exports = closingCashRepo;
