...
const { TRAY_MASTER } = require("../../catalog/traymaster/commons/constants")
const { UNITS } = require("../../catalog/units/commons/constants");
const { WASTAGE_HDR, WASTAGE_DTL, STOCKLEDGER, DNE_STOCK_LEDGER, FVWASTAGE_TRAY_DTL } = require("../commons/constants")
...
function fmcgWastageRepo(fastify) {
    ...
    return knex.transaction(async trx => {
      ...
      if (body.wastage_details?.length > 0) {
        const wastageDetailsData = body.wastage_details.map(detail => ({
          ...
          [WASTAGE_DTL.COLUMNS.WD_SUPP_ID]: detail.wd_suppid,
          [WASTAGE_DTL.COLUMNS.WD_REASON_ID]: detail.wd_reason_id,
          [WASTAGE_DTL.COLUMNS.WD_TRAY]: detail.wd_tray,
          [WASTAGE_DTL.COLUMNS.WD_TRAYCOUNT]: detail.wd_traycount,
      ...
      // Batch Insert for FVWASTAGE_TRAY_DTL
      if (body.wastage_tray_details?.length > 0) {
        const wastageDetailsData = body.wastage_tray_details.map(detail => ({
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_ID]: purchasePpId,
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_YEAR]: financialYear || detail.wt_year,
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_DATE]: detail.wt_date || new Date(),
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_SLNO]: detail.wt_slno,
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_TRAYID]: detail.wt_trayid,
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_QTY]: detail.wt_qty,
          [FVWASTAGE_TRAY_DTL.COLUMNS.WT_COM_ID]: userDetails.company_id,
          [FVWASTAGE_TRAY_DTL.COLUMNS.CREATED_BY]: userDetails.id,
          [FVWASTAGE_TRAY_DTL.COLUMNS.UPDATED_BY]: userDetails.id
        }));
        await batchInsertData(FVWASTAGE_TRAY_DTL.NAME, wastageDetailsData);
      }
      return { success: true };
    });
  }
  ...
  async function reduceStockLedger({ params, body, logTrace, userDetails, financialYear }) {
    ...
    return knex.transaction(async trx => {
      ...
      const productPlanRequests = await trx(WASTAGE_DTL.NAME)
        .select(WASTAGE_DTL.COLUMNS.WD_PRDID, WASTAGE_DTL.COLUMNS.WD_QTY)
      ...
      if (productPlanRequests.length > 0) {
        const stockLedgerData = productPlanRequests.map(detail => {
          ...
          const wasteQty = parseFloat(detail[WASTAGE_DTL.COLUMNS.WD_QTY]);
  ...
  async function putWastage({ params, body, logTrace, userDetails, financialYear }) {
    ...
    return knex.transaction(async trx => {
      ...
      await trx(WASTAGE_DTL.NAME).where(WASTAGE_DTL.COLUMNS.WD_ID, purchasePpId).del();
      await trx(FVWASTAGE_TRAY_DTL.NAME).where(FVWASTAGE_TRAY_DTL.COLUMNS.WT_ID, purchasePpId).del();
  ...
  async function getWastage({ queryString, params, logTrace }) {
    const knex = this;
    const { status, search, wastage_no, from_date, to_date } = queryString;
    ...
    const responsewith_details = await Promise.all(
      response.data.map(async offers => {
        ...
        const wastage_tray_details = await knex
          .select([
            `${FVWASTAGE_TRAY_DTL.NAME}.*`,
            `${TRAY_MASTER.NAME}.${TRAY_MASTER.COLUMNS.TRAY_NAME}`,
          ])
          .from(`${FVWASTAGE_TRAY_DTL.NAME} as ${FVWASTAGE_TRAY_DTL.NAME}`)
          .leftJoin(
            `${TRAY_MASTER.NAME} as ${TRAY_MASTER.NAME}`,
            `${FVWASTAGE_TRAY_DTL.NAME}.${FVWASTAGE_TRAY_DTL.COLUMNS.WT_TRAYID}`,
            `${TRAY_MASTER.NAME}.${TRAY_MASTER.COLUMNS.ID}`
          )
          .where(`${FVWASTAGE_TRAY_DTL.NAME}.${FVWASTAGE_TRAY_DTL.COLUMNS.WT_ID}`, offers.w_id);



        return { ...offers, wastage_details, wastage_tray_details };
      })
    );
  ...
  async function getWastageInfo({ params, logTrace }) {
    ...
    if (!response.data.length) {
      throw CustomError.create({
        httpCode: StatusCodes.NOT_FOUND,
        message: "FV wastage info not found",
        property: "",
        code: "NOT_FOUND"
      });
    }
  ...
  async function deleteWastage({ params, body, logTrace, userDetails, financialYear }) {
    ...
    return knex.transaction(async trx => {
      ...
      const query3 = trx(FVWASTAGE_TRAY_DTL.NAME).where(FVWASTAGE_TRAY_DTL.COLUMNS.WT_ID, purchasePpId).del();
      await query3;
      logQuery({
        logger: fastify.log,
        query: query3,
        context: "Delete fmcg wastage tray detail",
        logTrace
      });
      return { success: true };
    });
  ...
  async function postWastageList({ logTrace, body }) {
    const knex = this;

    // Fetch tray info
    const trayResult = await knex(TRAY_MASTER.NAME)
      .select(TRAY_MASTER.COLUMNS.ID, TRAY_MASTER.COLUMNS.TRAY_WEIGHT, TRAY_MASTER.COLUMNS.TRAY_NAME)
      .where(TRAY_MASTER.COLUMNS.ID, body.tray_id)
      .first();

    // Fetch item info
    const itemResult = await knex(ITEM.NAME)
      .select(ITEM.COLUMNS.ID, ITEM.COLUMNS.PRODUCT_NAME, ITEM.COLUMNS.MRP, ITEM.COLUMNS.UOM_ID, ITEM.COLUMNS.GST, ITEM.COLUMNS.CESS)
      .where(ITEM.COLUMNS.ID, body.item_id)
      .first();

    // Fetch UOM name using item’s uom_id
    const uomResult = await knex(UNITS.NAME)
      .select(UNITS.COLUMNS.UNITS_SHORT_NAME, UNITS.COLUMNS.UNITS_LONG_NAME)
      .where(UNITS.COLUMNS.ID, itemResult.uom_id)
      .first();
    console.log('itemResult:', itemResult);

    return {
      item_id: itemResult.id,
      tray_id: trayResult.id,
      tray_weight: Number(trayResult.tray_weight),
      tray_name: trayResult.tray_name,
      item_name: itemResult.pro_name,
      item_rate: Number(itemResult.mrp),
      item_gst: Number(itemResult.gst),
      item_cess: Number(itemResult.cess),
      uom_name: uomResult?.units_short_name
    };
  }
